...
// A transaction neither synced nor seen before is fetched and stored by main
// (main/sync/txs.js), with each referenced output, so it has enough context
// to show input addresses/values and calculate its fee.
const loadTransaction = async (txHash) => {
    ...
    return await window.electron.fetchTransaction(txHash)
}
