...
const DirectTx = async (inputs, outputs, beatHash, setModal) => {
    ...
    const onCorrectPassword = async () => {
        ...
        const tx = bitcoin.Transaction.fromBuffer(outer_txInfo.raw)
        let txb
        let txBuild
        for (let lockTime = 5e8; lockTime < 5e8 + 2 ^ 12; lockTime++) {
            txb = bitcoin.TransactionBuilder.fromTransaction(tx)
            for (let i = 0; i < outer_txInfo.inputs.length; i++) {
                const input = outer_txInfo.inputs[i]
            ...
            txBuild = txb.build()
            if (!outer_beatHash.current) {
                break
            }
            const txHash = txBuild.getId()
            if (txHash < outer_beatHash.current) {
                break
            }
            tx.locktime = lockTime
        }
        const buf = txBuild.toBuffer()
        outer_txInfo.raw = buf
